...
while True:
    num_rows = input('Введите количество строк: ')
    num_columns = input('Введите количество столбцов: ')
    if num_rows.isdigit() == False or num_columns.isdigit() == False or int(num_rows) <= 0 or int(num_columns) <= 0:
        print('Неверные данные, повторите ввод')
    else:
       break

print_operation_table(lambda x,y: x*y, int(num_rows), int(num_columns))